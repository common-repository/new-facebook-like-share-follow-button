...
class FbSocialShareButton extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array( 
			'classname' => __('Facebook Share Button', NSMW_TXTDM), 
			'description' => __('This is facebook share widget you can add this in a widget area or wherever you want it is very easy to use.', NSMW_TXTDM), 
		);
	...
	public function widget( $args, $instance ) {
		...
		if($url_type == "dynamic") {
			...
			$url = ! empty( $instance['url'] ) ? $instance['url'] : __( 'https://www.facebook.com/awordpresslife', 'text_domain' );
		}
		$url = ! empty( $instance['url'] ) ? $instance['url'] : '';
		$url_type = ! empty( $instance['url_type'] ) ? $instance['url_type'] : '';
		$layout = ! empty( $instance['layout'] ) ? $instance['layout'] : '';
		$button_size = ! empty( $instance['button_size'] ) ? $instance['button_size'] :  '';
		$mobile_frame = ! empty( $instance['mobile_frame'] ) ? $instance['mobile_frame'] : '';
		$language = ! empty( $instance['language'] ) ? $instance['language'] : __( 'en_US');
		...
		<div class="fb-share-button" data-toggle="tooltip" title="Share on facebook!"  data-href="<?php echo $url; ?>" data-layout="<?php echo $layout; ?>" data-size="<?php echo $button_size; ?>" data-mobile-iframe="<?php echo $mobile_frame; ?>">
			<a class="fb-xfbml-parse-ignore" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=<?php echo urlencode($url); ?>&amp;src=sdkpreparse"><?php _e('Share', NSMW_TXTDM); ?></a>
		</div>
		<?php
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'url_type' ); ?>"><?php _e( 'Share URL', NSMW_TXTDM); ?></label><br>
			<input class="widefat dynamic" id="<?php echo $this->get_field_id( 'url_type' ); ?>" name="<?php echo $this->get_field_name( 'url_type' ); ?>" <?php if($url_type == 'dynamic') echo "checked=checked" ?> type="radio" value="dynamic"><?php _e('Dynamic', NSMW_TXTDM); ?> &nbsp;&nbsp;
			<input class="widefat static" id="<?php echo $this->get_field_id( 'url_type' ); ?>" name="<?php echo $this->get_field_name( 'url_type' ); ?>" <?php if($url_type == 'static') echo "checked=checked" ?> type="radio" value="static"><?php _e('Static', NSMW_TXTDM); ?>
		</p>
		<p class="url-feild" <?php if($url_type == "dynamic") { ?> style="display:none"; <?php } ?>>
			<label for="<?php echo $this->get_field_id('url'); ?>"><?php _e('Url to share', NSMW_TXTDM); ?><input class="widefat" id="<?php echo $this->get_field_id('url'); ?>" name="<?php echo $this->get_field_name('url'); ?>" type="text" value="<?php echo $url; ?>" />
			</label>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'layout' ) ); ?>"><?php _e('Button Layout', NSMW_TXTDM); ?></label> 
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'layout' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'layout' ) ); ?>">
				<option value="box_count" <?php if($layout == 'box_count' ) echo "selected=selected"; ?>><?php _e('Box Count', NSMW_TXTDM); ?></option>
				<option value="button_count" <?php if($layout == 'button_count' ) echo "selected=selected"; ?>><?php _e('Button Count', NSMW_TXTDM); ?></option>
				<option value="button" <?php if($layout == 'button' ) echo "selected=selected"; ?>><?php _e('Button', NSMW_TXTDM); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'button_size' ) ); ?>"><?php _e('Button Size', NSMW_TXTDM); ?></label> 
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'button_size' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'button_size' ) ); ?>">
				<option value="small" <?php if($button_size == 'small' ) echo "selected=selected"; ?> ><?php _e('Small', NSMW_TXTDM); ?></option>
				<option value="large" <?php if($button_size == 'large' ) echo "selected=selected"; ?>><?php _e('Large', NSMW_TXTDM); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'mobile_frame' ); ?>"><?php _e('Enable Mobile Frame', NSMW_TXTDM); ?></label><br>
			<input class="widefat" id="<?php echo $this->get_field_id( 'mobile_frame' ); ?>" name="<?php echo $this->get_field_name( 'mobile_frame' ); ?>" <?php if($mobile_frame == 'true') echo "checked=checked" ?> type="radio" value="true"><?php _e('Yes', NSMW_TXTDM); ?> &nbsp;&nbsp;
			<input class="widefat" id="<?php echo $this->get_field_id( 'mobile_frame' ); ?>" name="<?php echo $this->get_field_name( 'mobile_frame' ); ?>" <?php if($mobile_frame == 'false') echo "checked=checked" ?> type="radio" value="false"><?php _e('No', NSMW_TXTDM); ?>
		</p>
		<p>
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['url'] = ( ! empty( $new_instance['url'] ) ) ? strip_tags( $new_instance['url'] ) : 'https://www.facebook.com/awordpresslife';
